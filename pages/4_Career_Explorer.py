...
st.set_page_config(page_title='Career Explorer')
...
if query:
    q = query.lower()
    results = [c for c in CAREERS if q in c['name'].lower() or any(q in s.lower() for s in c.get('keywords',[])) or any(q in s.lower() for s in c.get('skills',[]))]
else:
    results = CAREERS[:20]
